 ...
 *
 * Provides ability to access and modify object and array elements using a string representation of the
 * the property call path.
 *
 * A path is a string representation of an Object or Array property call path.
 * Object property names and array index values are seperated with a dot '.' to form a string. 
...
class ObjectPathAccessor
{
  ...
  * Mutate path
  *
  * Mutates all elements matching 'path' in 'subjec' using 'mutator' function.
  * Returns all matching elements with updated values.
  */
   ...
   * Matched values maye be modifed using optional mutatorFunc.
   * Returns matched value(s). 
   */
  static searchRescursive(pattern, subject, mutatorFunc, currentPath = '', currentDepth = 1, matches = []) {
    var currentParts = pattern.split('.').slice(0, currentDepth); // Pattern parts up to current depth
    var currentPattern = currentParts.join('.'); // Pattern string up to current depth
    ...
    function processElement(prop)
    {
      ...
      var elementPath = currentPath.length ? currentPath + '.' + prop : prop;
      if (typeof elementPath == 'number') elementPath = elementPath.toString(); // Element path must be a string
      // Element pattern is the full pattern with wilcards replaced by actual values
      // - this alows us to compare the full pattern with current path
      var elementPattern = (currentNode == '*') ? pattern.replace(currentPattern, elementPath) : pattern;
      
      if (elementPattern == elementPath) {
      ...
      if (subject[prop] === Object(subject[prop])) ObjectPathAccessor.searchRescursive(elementPattern, subject[prop], mutatorFunc, elementPath, currentDepth, matches);
    }
