...
class Repo
{
  ...
  {
    if (findOptions.options['stripPrivate']) {
      this.schema.stripPrivate(objects, 'read');
    }
  ...
  insertMany(objects, options)
  {
    ...
    if (options && options['stripPrivate']) {
      this.schema.stripPrivate(args[0], 'write');
    }
  ...
  updateMany(criteria, update, options)
  {
    ...
    if (update && update['$set'] && options && options['stripPrivate'] ) {
      this.schema.stripPrivatePaths(update['$set'], 'write');
    }
