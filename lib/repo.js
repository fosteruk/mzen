...
class Repo
{
  ...
  find()
  {
    ...
    const validateResult = this.schema.validateQuery(findOptions.query);
  ...
  {
    const findOptions = this.findQueryOptions(arguments);
    const validateResult = this.schema.validateQuery(query);
  ...
  insertMany(objects, options)
  {
    ...
    var promise = new Promise((resolve, reject) => {
      let validateResult = this.schema.validate(args[0]);
  ...
  updateMany(criteria, update, options)
  {
    ...
    var promise = new Promise((resolve, reject) => {
      try {
        ...
        let validateResultQuery = this.schema.validateQuery(criteria);
        let validateResultUpdate = {};
        if (update && update['$set']) {
           validateResultUpdate = this.schema.validatePaths(update['$set']);
        }
        let validateResult = Schema.mergeValidationResults([validateResultQuery, validateResultUpdate]);
  ...
  updateOne(criteria, update, options)
  {
    ...
    var promise = new Promise((resolve, reject) => {
      ...
      const validateResult = Schema.mergeValidationResults(validationResults);
