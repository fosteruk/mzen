...
const repo_populate_1 = require("./repo-populate");
...
exports.RepoErrorValidation = RepoErrorValidation;
class Repo {
    constructor(options = {}) {
        this.config = options;
    ...
    initSchema() {
        if (!this.schema) {
            ...
            if (this.config.schema instanceof mzen_schema_1.default) {
                this.schema = this.config.schema;
            }
            else if (this.config.schema && Object.keys(this.config.schema).length) {
                this.schema = new mzen_schema_1.default(this.config.schema);
            }
                ...
                this.schema = new mzen_schema_1.default;
            }
    ...
    getName() {
        ...
        if (this.name == 'Repo')
            throw new Error('Repo name not configured - you must specify a repo name when using the default repo entityConstructor');
        return this.name;
    }
    ...
    createIndexes() {
        ...
        for (let indexName in this.config.indexes) {
            if (!this.config.indexes.hasOwnProperty(indexName))
                continue;
    ...
    find() {
        return __awaiter(this, arguments, void 0, function* () {
            ...
            var validateResult = yield this.schema.validateQuery(findOptions.query);
            ...
            var objects = yield this.dataSource.find(this.config.collectionName, findOptions.query, findOptions.fields, findOptions.queryOptions);
    ...
    findOne() {
        return __awaiter(this, arguments, void 0, function* () {
            ...
            // @ts-ignore - Expected 0 arguments, but got 2 - variable method arguments
            var objects = yield this.dataSource.findOne(this.config.collectionName, findOptions.query, findOptions.fields, findOptions.queryOptions);
            return this.findPopulate(objects, findOptions);
        });
    }
    count(query, options) {
        return __awaiter(this, void 0, void 0, function* () {
            this.initSchema();
            var validateResult = yield this.schema.validateQuery(query);
            if (!validateResult.isValid) {
    ...
    findQueryOptions(findArguments) {
        ...
        if (options.collation != undefined)
            queryOptions.collation = options.collation;
        ...
        if (options.entityConstructor !== false)
            delete options.entityConstructor;
    ...
    populate(relation, objects, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.getPopulatePromise(relation, options, objects); // side affect modified objects
            return objects;
        });
    }
    getFlattenedRelations(options) {
        ...
        flattenedRelationsRecursive.forEach(function (relationConfig) {
            ...
            if (flattenedRelations[depth] == undefined)
                flattenedRelations[depth] = [];
    ...
    getFlattenedRelationsRecursive(options = {}, parentRepo, basePath = [], flatRelations = []) {
        ...
        for (var alias in this.config.relations) {
            ...
            const relation = clone_1.default(this.config.relations[alias]); // copy the relation we dont want to modify the original
    ...
    getPopulatePromise(relation, options = {}, objects = []) {
        ...
        var relationOptions = clone_1.default(options);
        ...
        var repoPopulate = new repo_populate_1.default(repo);
    ...
    insertMany(objects, options) {
        return __awaiter(this, arguments, void 0, function* () {
            ...
            var validateResult = yield this.schema.validate(args[0]);
            if (!validateResult.isValid) {
                throw new RepoErrorValidation(validateResult.errors);
            }
    ...
    updateMany(criteria, update, options) {
        return __awaiter(this, void 0, void 0, function* () {
            this.initSchema();
            criteria = clone_1.default(criteria);
            update = clone_1.default(update);
            var promises = [];
            var validateResult = {};
            ...
            validateResult = mzen_schema_1.default.mergeValidationResults([validateResultQuery, validateResultUpdate]);
            if (!validateResult.isValid) {
                throw new RepoErrorValidation(validateResult.errors);
            }
    ...
    updateOne(criteria, update, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield Promise.all(promises);
            var validateResult = mzen_schema_1.default.mergeValidationResults(validationResults);
            if (!validateResult.isValid) {
                throw new RepoErrorValidation(validateResult.errors);
            }
    ...
    deleteMany(filter, options) {
        return __awaiter(this, arguments, void 0, function* () {
            ...
            var validateResult = yield this.schema.validateQuery(filter);
            if (!validateResult.isValid) {
    ...
    stripTransients(objects) {
        this.initSchema();
        var newObjects = clone_1.default(objects); // We will be deleting relations so we need to work on a copy
        ...
        this.relationPaths.forEach(function (relationPath) {
            object_path_accessor_1.default.unsetPath('*.' + relationPath, newObjects);
        });
