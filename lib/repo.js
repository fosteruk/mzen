...
class Repo
{
  constructor(options = {})
  {
    ...
    for (let alias in this.config.relations) {
      ...
      let docPath = this.config.relations[alias].docPath ? this.config.relations[alias].docPath : null;
      let aliasPath = docPath ? docPath + '.' + this.config.relations[alias].alias : this.config.relations[alias].alias;
  ...
  async findPopulate(objects, findOptions)
  {
    ...
    objects = objects ? this.schema.applyConstructors(objects, findOptions) : objects;
  ...
  async populate(relation, objects, options)
  {
    ...
    var populateResult = await this.getPopulatePromise(relation, options, objects); // side affect modified objects
    return objects;
  }
  ...
  async updateMany(criteria, update, options)
  {
    ...
    var validateResults = Schema.mergeValidationResults([validateResultQuery, validateResultUpdate]);
    if (!validateResults.isValid) {
      throw new RepoErrorValidation(validateResults.errors);
    }
  ...
  stripTransients(objects)
  {
    ...
    newObjects.forEach((newObject, x) => {
      this.relationPaths.forEach(function(relationPath, y){
        // Todo: update this to handle full paths not just single alias name
        if (newObjects[x][relationPath]) delete newObjects[x][relationPath];
      });
    });
