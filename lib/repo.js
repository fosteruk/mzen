...
class Repo
{
  ...
  applyConstructors(objects, findOptions)
  {
    ...
    if (findOptions.queryOptions['entityConstructor'] !== false) {
      ...
      if (Object.keys(constructors).length) {
        for (let docPath in constructors) {
          if (!constructors.hasOwnProperty(docPath)) continue;
          var construct = constructors[docPath];
          ...
          const path = docPath.length ? '*.' + docPath : '*';
          ObjectPathAccessor.mutatePath(path, objects, function(rawObject){
            const object = Object.create(construct.prototype);
  ...
  getFlattenedRelationsRecursive(options = {}, parentRepo, basePath = [], flatRelations = [])
  {
    ...
    for (var alias in this.config['relations']) {  
      ...
      relation.docPath = basePath.length ? basePath.join('.') : '';
      const flatRelation = {
        depth: depth,
        path: relation.docPath.length ?  relation.docPath + '.' + relation.alias : relation.alias,
  ...
  getPopulatePromise(relation, options = {}, objects = [])
  {
    ...
    relationOptions['docPath'] = relation['docPath'];
  ...
  updateMany(criteria, update, options)
  {
    ...
    if (update && update['$set'] && options && options['filterPrivate'] ) {
      this.schema.filterPrivatePaths(update['$set'], 'write');
    }
    ...
    return Promise.all(promises).then(() => {
      ...
      if (!validateResults.isValid) {
        return Promise.reject(new RepoErrorValidation(validateResults.errors));
      }
