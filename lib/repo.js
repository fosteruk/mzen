...
class Repo
{
  ...
  { 
    ...
    for (let x in flattenedRelationsRecursive) {
      var depth = flattenedRelationsRecursive[x].depth;
      if (flattenedRelations[depth] == undefined) flattenedRelations[depth] = [];
      flattenedRelations[depth].push(flattenedRelationsRecursive[x].relation);
    }
    
    return flattenedRelations;
  }
  getFlattenedRelationsRecursive(options = {}, basePath = [], flatRelations = [])
  {
    for (var alias in this.config['relations']) {  
      ...
      var populate = (options['populate'][alias] !== undefined) ? options['populate'][alias] : populate;
      var recursion = relation['recursion'] ? relation['recursion'] : 0;

      if (!populate) continue;
      if (depth > recursion) continue;
      ...
      if (relation.type != 'hasManyCount') {
        ...
        flatRelations.concat(repo.getFlattenedRelationsRecursive(options, newBasePath, flatRelations));
      }
    }
