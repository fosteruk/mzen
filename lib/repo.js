...
var TypeCaster = require('mzen-schema/lib/type-caster');
...
class Repo
{
  constructor(options = {}) 
  {
    ...
    for (let alias in this.config['relations']) {
  ...
  reset()
  {
    ...
    return this.drop().then(() => {
      if (this.config['autoIndex']) return this.createIndexes();
    });
  }
  createIndexes()
  {
  ...
  find()
  {
    ...
    return this.schema.validateQuery(findOptions.query).then((validateResult) => {
  ...
  {
    const findOptions = this.findQueryOptions(arguments);
    return this.schema.validateQuery(query).then((validateResult) => {
      if (!validateResult.isValid) {
        return Promise.reject(new RepoErrorValidation(validateResult.errors));
      }
  ...
  findQueryOptions(findArguments)
  {
    ...
    options['stripPrivateFields'] = (options['stripPrivateFields'] !== undefined)  ? options['stripPrivateFields'] : false;
  ...
  applyConstructors(objects, findOptions)
  {
    ...
    if (findOptions.queryOptions['entityConstructor'] !== false) {
      ...
      if (Object.keys(constructors).length) {
        for (let documentPath in constructors) {
          ...
          const path = documentPath.length ? '*.' + documentPath : '*';
  ...
  {
    if (findOptions.options['stripPrivateFields'] === true || findOptions.options['stripPrivateFields'] == 'read') {
  ...
  getFlattenedRelationsRecursive(options = {}, parentRepo, basePath = [], flatRelations = [])
  {
    ...
    for (var alias in this.config['relations']) {  
      const depth = (basePath.depth == undefined) ? 0 : basePath.depth;
      const relation = clone(this.config['relations'][alias]); // copy the relation we dont want to modify the original
      const populate = (relation['populate'] !== undefined) ? relation['populate'] : false;
  ...
  insertMany(objects, options)
  {
    ...
    if (options['stripPrivateFields'] === true || options['stripPrivateFields'] == 'write') {
      args[0] = this.schema.stripPrivateFields(args[0], 'write');
    }

    return this.schema.validate(args[0]).then((validateResult) => {
      if (!validateResult.isValid) {
        return Promise.reject(new RepoErrorValidation(validateResult.errors));
      }
      
      args.unshift(this.config['collectionName']); // Prepend collection name to arguments
      return this.dataSource.insertMany.apply(this.dataSource, args).then(function(result){
        return args[0];
      });
    });
  }
  insertOne(object, options)
  {
    ...
    if (options && (options['stripPrivateFields'] === true || options['stripPrivateFields'] == 'write')) {
      args[0] = this.schema.stripPrivateFields(args[0], 'write');
    }
    ...
    return this.schema.validate(args[0]).then((validateResult) => {
      ...
      args.unshift(this.config['collectionName']); // Prepend collection name to arguments
      return this.dataSource.insertOne.apply(this.dataSource, args).then(function(result){
        return args[1];
      })
    });
  }
  updateMany(criteria, update, options)
  {
    ...
    Promise.all(promises).then(() => {
      validateResults = Schema.mergeValidationResults([validateResultQuery, validateResultUpdate]);
      ...
      args.unshift(this.config['collectionName']); // Prepend collection name to arguments
      return this.dataSource.updateMany.apply(this.dataSource, args).then(function(result){
        resolve(args[1]);
      });
    })
  }
  updateOne(criteria, update, options)
  {
    ...
    return Promise.all(promises).then(() => {
      let validateResult = Schema.mergeValidationResults(validationResults);
      ...
      args.unshift(this.config['collectionName']); // Prepend collection name to arguments
      return this.dataSource.updateOne.apply(this.dataSource, args);
    });
  }
  deleteMany(filter, options)
  {
    filter = filter ? filter : {};
    return this.schema.validateQuery(filter).then((validateResult) => {
      if (!validateResult.isValid) {
  ...
  stripTransients(objects)
  {
    ...
    newObjects.forEach((newObject, x) => {
