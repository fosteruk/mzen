...
class RepoPopulate
{
  ...
  has(docs, options)
  {
    ...
    var relationIds = RepoPopulate.prototype.getRelationIds(docs, options);
    ...
    return this.relationRepo.find(options.query, options).then(function(relatedDocs){
  ...
  getRelationIds(docs, options)
  {
    ...
    // - we want to deal with both situations uniformly so if we are passed a sigle object we make it an array
    var docs = Array.isArray(docs) ? docs : [docs];
    var relationIds = [];
    ...
    var documentPath = options.documentPath ? '*.' + options.documentPath : '*';
    var embeddedDocs = ObjectPathAccessor.getPath(documentPath, docs);
    
    embeddedDocs.forEach(function(doc, x){
      if (Array.isArray(doc[options.sourceKey])) {
        // Only belongsToMany relation supports this
