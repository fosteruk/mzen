...
class RepoPopulate
{
  ...
  embeddedHas(docs, options)
  {
    ...
    const relationIds = RepoPopulate.prototype.getRelationIds(docs, options);
    const embeddedDocs = this.getEmebedRelations(options.documentPathRelated, docs);
  ...
  embeddedBelongsTo(docs, options)
  {
    ...
    const relationIds = RepoPopulate.prototype.getRelationIds(docs, options);
    const embeddedDocs = ObjectPathAccessor.getPath(options.documentPathRelated, docs);

    var values = {};
    embeddedDocs.forEach((embeddedDoc, x) => {
      if (relationIds.indexOf(embeddedDoc[options.pkey]) != -1) {
        // Group related docs by parent key
        values[embeddedDocs[x][options.pkey]] = embeddedDocs[x];
      }
    });
    ...
    embeddedDocs.forEach(function(doc, x){
      ...
      if (sourceKey) {
        if (Array.isArray(sourceKey)) {
          ...
          doc[options.alias] = isHasOne ? values[sourceKey][0] : values[sourceKey];
        }
      }
    });
  ...
  normalizeOptions(options)
  {
    ...
    // SsourceKey is an internal option which refers to either the key or the pkey depending on relation type
    let isBelongsTo = (options['relation'].toLowerCase().indexOf('belongsto') != -1);
    options.sourceKey = isBelongsTo ? options['key'] : options['pkey'];

    // alias is the field name used to store the copiled relations 
    options.alias = options['alias'] ? options['alias'] : '';
    // documentPath is path to the object(s) where the relation should be populated
    options.documentPath = options['documentPath'] ? options['documentPath'] : false;
    // documentPathRelated is the path to the related objects
    options.documentPathRelated = options['documentPathRelated'] ? options['documentPathRelated'] : false;
