...
class Service
{
  ...
  {
    var method = this.getActionMethod(actionName);
  ...
  prepRequest(request, action)
  {
    ...
    if (action) {
      var actionConfig = this.config['action'][action];
      if (actionConfig) {
  ...
  applySchemaValiation(actionConfig, request)
  {
    if (actionConfig && actionConfig['schema']) {
      var schema = new Schema(actionConfig['schema']);
  ...
  prepSchema(schema)
  {
    var getRepoSchema = function(schemaRepoDescriptor) {
      var repoName = schemaRepoDescriptor.constructor == String ? schemaRepoDescriptor : schemaRepoDescriptor['name'];
      var repoPath = schemaRepoDescriptor.constructor == Object ? schemaRepoDescriptor['path'] : '';
      
      var repo = this.getRepo(repoName);
  ...
  autoloadParams(actionConfig, request)
  {
    ...
    if (actionConfig && actionConfig['autoload']) {
      ...
      for (var paramName in actionConfig['autoload']) {
        ...
        var requestValue = request[paramName];
        if (requestValue == undefined) continue; // Param was not provided in the request - nothing to do here
        
        var isArray = Array.isArray(requestValue);
        var ids = isArray ? requestValue : [requestValue];
        var paramConfig = actionConfig['autoload'][paramName];
        ...
        var limit = paramConfig['limit'] ? paramConfig['limit'] : 1000;
